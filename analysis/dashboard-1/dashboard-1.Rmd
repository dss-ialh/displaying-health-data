---
title: "Cog vs Phys by County --PDSA Cycle 3"
output:
...
### Notes
1. The current report covers `r scales::comma(nrow(ds_provider_week))` total monthly records for `r dplyr::n_distinct(ds_provider_week$program_code[ds_provider_week$program_code == 755])` unique programs and `r dplyr::n_distinct(ds_provider_week$model_name)` unique models (`r paste(sort(unique(ds_provider_week$model_name)), collapse=", ")`).
1. The dates range from `r min(ds_provider_week$week, na.rm=T)` to  `r max(ds_provider_week$week, na.rm=T)`.
1. The selected program is `r program_focus`.
1. The PDSA Cycle 3 start date is `r as.Date("2018-08-01")`
...
### Tables - Client-week
```{r tables-client-week, echo=echo_chunks, message=message_chunks}
```

### Tables - Provider-Week

```{r tables-provider-week, echo=echo_chunks, message=message_chunks}
```
...
<!-- A program's monthly values are plotted on top of the background of other programs in the model.  Each graph represents one outcome. -->
...
#### SMART Aim
By implementing plans for self-care activities with established clients, primary caregivers will see a mean score reduction of at least 4 points on the Parenting Stress Scale after 4 months of self-care activities by 12-1-18.
...
* [Current PDSA]https://docs.google.com/document/d/1IcwCCkr1EkoeBJpEKdF2v-E0gKI6wGacCv9oZU1CE7I/edit#
